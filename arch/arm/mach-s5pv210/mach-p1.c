...
static cntry_locales_custom_t crespo_wifi_translate_custom_table[] = {
...
{
	int size = ARRAY_SIZE(crespo_wifi_translate_custom_table);
	int i;
	...
	for (i = 0; i < size; i++)
		if (strcmp(ccode, crespo_wifi_translate_custom_table[i].iso_abbrev) == 0)
			return &crespo_wifi_translate_custom_table[i];
	return &crespo_wifi_translate_custom_table[0];
}
